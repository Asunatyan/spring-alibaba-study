...
public class DistroMapper extends MemberChangeListener {
    ...
    public boolean responsible(String serviceName) {
        ...
        //多个集群只有一个返回true,其他的节点不会执行健康检测任务
        //hash运算出一个固定值,如果一台机器挂了呢? 关注下面的两个任务,来保证hash的正确性
        //注册服务实例信息在集群节点间同步任务 集群节点(健康状态)状态同步任务 ServiceReporter.run) ServiceStutasReporter.run()
